...
def inspector_gui(m: geemap.Map | None = None):
    ...
    def handle_interaction(**kwargs):
        ...
        if (
            ...
            with output:
                ...
                layer_dict = m.cog_layer_dict[  # pyrefly: ignore[missing-attribute]
                    dropdown.value
            ...
            if layer_dict["type"] == "STAC":
                ...
                if result is not None:
                    with output:
                        ...
                        m.pixel_values.append(  # pyrefly: ignore[missing-attribute]
                            result
                        )  # pyrefly: ignore[missing-attribute]
                    if add_marker.value:
                        markers = list(
    ...
    if m is not None:
        if not hasattr(m, "marker_cluster"):
            setattr(
                m, "marker_cluster", marker_cluster  # pyrefly: ignore[unbound-name]
        ...
        if change["new"] == "Convert":
            ...
            if len(text_widget.value) > 0:
                ...
                if (
                    ...
                    and len(
                        out_lines[0].strip()  # pyrefly: ignore[unsupported-operation]
                    )  # pyrefly: ignore[unsupported-operation]
                    == 0
                ):
                ...
                text_widget.value = "".join(
                    [prefix] + out_lines  # pyrefly: ignore[unsupported-operation]
                )  # pyrefly: ignore[unsupported-operation]
                coreutils.create_code_cell(
                    "".join(out_lines)  # pyrefly: ignore[no-matching-overload]
        ...
        if change["new"]:
            ...
            if colormap.value is not None:
                ...
                if m._colorbar_widget is None:  # pyrefly: ignore[missing-attribute]
                    m._colorbar_widget = (  # pyrefly: ignore[missing-attribute]
                ...
                if (not hasattr(m, "_colorbar_ctrl")) or (m._colorbar_ctrl is None):
                    ...
                    m.add_control(  # pyrefly: ignore[missing-attribute]
    ...
    def colormap_changed(change) -> None:
        if change["new"]:
            ...
            if hasattr(m, "_colorbar_ctrl") or (
                m._colorbar_ctrl is None  # pyrefly: ignore[missing-attribute]
            ):  # pyrefly: ignore[missing-attribute]
                m._colorbar_ctrl = (  # pyrefly: ignore[missing-attribute]
            ...
            if selected in m.ee_layers:  # pyrefly: ignore[missing-attribute]
                ...
                if isinstance(ee_object, ee.Image):
                    ...
                    if band_count > 2:
                        ...
                        band1_dropdown.value = band_names[  # pyrefly: ignore[unsupported-operation]
                            2
                        ]  # pyrefly: ignore[unsupported-operation]
                        band2_dropdown.value = band_names[  # pyrefly: ignore[unsupported-operation]
                            1
                        ]  # pyrefly: ignore[unsupported-operation]
                        band3_dropdown.value = band_names[  # pyrefly: ignore[unsupported-operation]
                            0
                        ]  # pyrefly: ignore[unsupported-operation]
                ...
                if (
                    ...
                    band1_dropdown.value = bandnames[  # pyrefly: ignore[bad-index, unbound-name]
                        2

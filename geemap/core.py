...
class AbstractDrawControl:
    ...
    def last_feature(self) -> ee.Feature | None:
        ...
        property = self.get_geometry_properties(
            self.last_geometry  # pyrefly: ignore[bad-argument-type]
        )
    ...
    def _handle_geometry_deleted(self, geo_json: dict) -> None:
        ...
        if index >= 0:
            ...
            if self.count:
                ...
                _DRAWN_FEATURES_LAYER
                in self.host_map.ee_layers  # pyrefly: ignore[missing-attribute, union-attr]
            ):
                # Remove drawn features layer if there are no geometries.
                self.host_map.remove_layer(  # pyrefly: ignore[missing-attribute, union-attr]
                    _DRAWN_FEATURES_LAYER
                )
...
class Map(ipyleaflet.Map, MapInterface):  # pyrefly: ignore[inconsistent-inheritance]
    ...
    def _get_geometry(
        ...
        if isinstance(ee_object, ee.Geometry):
            return ee_object
        try:
            return ee_object.geometry(  # pyrefly: ignore[missing-attribute]
                maxError=max_error
            )
    ...
    def _add_legend(
        ...
        **kwargs: Any,
        ...
        if layer := self.ee_layers.get(  # pyrefly: ignore[no-matching-overload]
            layer_name, None
        ):
